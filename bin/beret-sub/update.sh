...
CHOICES=(
	"Beret        Update Beret itself and run any migrations"
...
if [[ "$CHOICE" == "<< Back"* ]] || [[ -z "$CHOICE" ]]; then
	...
	case "$INSTALLER" in
	"beret") INSTALLER_FILE="$BASE_DIR/bin/beret-sub/migrate.sh" ;;
	"ollama") INSTALLER_FILE="$BASE_DIR/install/terminal/optional/app-ollama.sh" ;;
	*) INSTALLER_FILE="$BASE_DIR/install/terminal/app-$INSTALLER.sh" ;;
	esac
